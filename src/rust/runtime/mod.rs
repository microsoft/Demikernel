...
impl SharedDemiRuntime {
    ...
    pub fn wait_any(
        ...
        let mut result = Err(Fail::new(libc::ETIMEDOUT, "wait timed out"));
        // Wait until one of the qts is ready.
        self.wait_next_n(
            |completed_qt, competed_qd, completed_result| {
                for (i, qt) in qts.iter().enumerate() {
                    if completed_qt == *qt {
                        result = Ok((i, completed_qt, competed_qd, completed_result));
                        return false;
                    }
                }
                true
    ...
    pub fn wait_next_n<Acceptor: FnMut(QToken, QDesc, OperationResult) -> bool>(
        ...
        // 2. None of the tasks have already completed, so start a timer.
        ...
        loop {
            // Run for one quanta and if one of our queue tokens completed, then if the acceptor says don't continue.
            if let Some((qt, qd, result)) = self.wait_next() {
                if acceptor(qt, qd, result) == false {
                    return Ok(());
                }
            }
    ...
    pub fn wait_next(&mut self) -> Option<(QToken, QDesc, OperationResult)> {
    ...
    pub fn poll_foreground_tasks(&mut self) {
        let foreground_group_id: TaskId = self.foreground_group_id;
        for boxed_task in self
            .scheduler
